...
struct TeleopArDrone
{
  ...
  std_srvs::Empty srv_empty;
    ...
    ROS_INFO("Hi, Found joystick with %zu buttons and %zu axes", joy_msg->buttons.size(), joy_msg->axes.size());
    if (joy_msg->buttons.size() != 13 || joy_msg->axes.size() != 8){
	 ...
	 twist.linear.x = -1*scale*joy_msg->axes[0]; // forward, backward
	 twist.linear.y = scale*joy_msg->axes[1]; // left right
   twist.linear.z = scale*joy_msg->axes[3]; // up down
   twist.angular.z = 0//scale*joy_msg->axes[2]; // yaw
